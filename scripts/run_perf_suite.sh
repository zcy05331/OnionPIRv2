...
#   tree      tree_bench_g32: Tree PIR MVP, 32 字节节点。
...
#             3x 复制)。
# tree/tree-g1/cuckoo 的树高、次数与种子都是运行时旗标
# (--leaf-count/--experiments/--warmup/--trial-seed), 二进制与档位无关,
# 全套只构建一次; 同一 TRIAL_SEED 驱动 merkle 与这三种基准的查询调度。
#
# 旋钮(环境变量):
...
#   JOBS=<nproc>    NOAVX512=OFF
#   SKIP_BUILD=0    =1 时复用 build-perf-suite/Onion-PIR 不重建
#                   (须与当前检出一致, 00_meta.txt 记录的 commit 以此为准)
#
# 异机准备:
#   1) git clone <repo> && cd OnionPIRv2 && git checkout <分支/commit>
#   2) 需要 cmake>=3.13、C++20 编译器(gcc>=13 / 近年 clang)、git;
...
#   00_meta.txt 00_cpu.txt 00_status.txt cpu_info.txt
#   merkle.txt merkle.json(内含 1GB 与 4GB 两档 case)
#   tree_g32_{1gb,4gb}.txt tree_g1_{1gb,4gb}.txt cuckoo_{1gb,4gb}.txt
#   compress.txt
# 完整性: 00_status.txt 每行 "<单元> ok|fail|skip [原因]"。任一单元失败
#   退出码 1; 全部通过但有请求的单元被资源门跳过(矩阵不完整)退出码 3;
#   只有全部单元 ok 才退出 0。单元失败不中断其余单元。
# =============================================================================
set -euo pipefail
...
mem_gate() {  # mem_gate <单元> <所需字节> -> 0 通过; 否则记 skip 并返回 1
  local unit="$1" need="$2"
  local margin=$((2 << 30))
  if [[ "${MEM_BYTES}" -gt 0 && $((need + margin)) -gt "${MEM_BYTES}" ]]; then
    echo "[skip] ${unit}: 粗估峰值 ${need}B + 2GB 余量 > 物理内存 ${MEM_BYTES}B" >&2
    record "${unit}" skip "peak_estimate=${need}B mem=${MEM_BYTES}B"
    return 1
  fi
  return 0
}
...
mkdir -p "${OUT_DIR}"
STATUS_FILE="${OUT_DIR}/00_status.txt"
: > "${STATUS_FILE}"
FAIL_COUNT=0
SKIP_COUNT=0
record() {  # record <单元> ok|fail|skip [原因]
  echo "$1 $2${3:+ $3}" >> "${STATUS_FILE}"
  case "$2" in
...
run_bin() {  # run_bin <日志名> <参数...>; 单元名 = 日志名去 .txt; 失败不终止套件
  local log="${OUT_DIR}/$1"; local unit="${1%.txt}"; shift
  echo "==> $* | tee ${log}"
  if ${CXX_LAUNCH[@]+"${CXX_LAUNCH[@]}"} "${BINARY}" "$@" 2>&1 | tee "${log}"; then
    record "${unit}" ok
  else
    echo "[fail] ${unit}: 退出非零, 详见 ${log}(套件继续)" >&2
    record "${unit}" fail "log=$(basename "${log}")"
  fi
}
...
MODES=("$@")
if [[ ${#MODES[@]} -eq 0 ]]; then
  MODES=(merkle tree tree-g1 compress cuckoo)
fi
...
WANT_4GB=0
case " ${SCALES} " in *" 4gb "*) WANT_4GB=1 ;; esac
...
# 2^26 档(套件内固定 4 次测量, 内存不够则套件自记 skipped_resource_limit,
# 本脚本从 merkle.json 读回该状态计入 00_status.txt)
if has_mode merkle; then
  ...
  [[ "${WANT_4GB}" == "1" ]] && MERKLE_ARGS+=(--run-optional-4gb)
  ...
  run_bin "merkle_${SCALE_LIST[0]}.txt" "${MERKLE_ARGS[@]}"
  ...
  if [[ "${WANT_4GB}" == "1" ]]; then
    if [[ ! -f "${OUT_DIR}/merkle.json" ]]; then
      record merkle_4gb fail "no merkle.json"
    elif grep -q '"skipped_resource_limit"' "${OUT_DIR}/merkle.json"; then
      echo "[skip] merkle_4gb: 套件内存门跳过了 2^26 档" >&2
      record merkle_4gb skip resource_limit
    elif grep -q '"status": "completed"' "${OUT_DIR}/merkle.json"; then
      record merkle_4gb ok
    else
      record merkle_4gb fail "optional row status missing"
    fi
  fi
fi
...
for SCALE in ${SCALES}; do
  ...
  SCALE_ARGS=(--leaf-count $((1 << LEAF_POW)) --experiments "${EXPERIMENTS}"
              --warmup "${WARMUPS}" --trial-seed "${TRIAL_SEED}")
  if has_mode tree; then
    if mem_gate "tree_g32_${SCALE}" $((1 << (LEAF_POW + 10))); then
      run_bin "tree_g32_${SCALE}.txt" --test tree_bench_g32 "${SCALE_ARGS[@]}"
    fi
  fi
  if has_mode tree-g1; then
    run_bin "tree_g1_${SCALE}.txt" --test tree_bench "${SCALE_ARGS[@]}"
  fi
  if has_mode cuckoo; then
    if mem_gate "cuckoo_${SCALE}" $((1 << (LEAF_POW + 10))); then
      run_bin "cuckoo_${SCALE}.txt" --test cuckoo_bench "${SCALE_ARGS[@]}"
    fi
  fi
done
...
echo "==> 结果目录: ${OUT_DIR}"
ls -1 "${OUT_DIR}"
echo "==> 单元状态(${STATUS_FILE}):"
cat "${STATUS_FILE}"
if [[ "${FAIL_COUNT}" -gt 0 ]]; then
  echo "==> 未完成: ${FAIL_COUNT} 个单元失败, ${SKIP_COUNT} 个被跳过" >&2
  exit 1
elif [[ "${SKIP_COUNT}" -gt 0 ]]; then
  echo "==> 矩阵不完整: ${SKIP_COUNT} 个请求的单元被资源门跳过" >&2
  exit 3
fi
echo "==> 完成: 全部单元 ok"
